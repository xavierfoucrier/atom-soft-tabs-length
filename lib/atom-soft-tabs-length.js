...
export default {
  ...
  observe() {
    ...
    // observe indent guide
    this.observer.add(atom.config.observe('editor.showIndentGuide', {}, (value) => {
      if (atom.config.get('atom-soft-tabs-length.hideIndentGuide') && value) {
        atom.notifications.addWarning('atom-soft-tabs-length', {
          description: 'The indent guide is currently hidden by the `atom-soft-tabs-length` package. You won\'t be able to see it until you turn off the `hideIndentGuide` option.',
          dismissable: true,
        });
      }
    }));
  ...
  run(editor) {
    ...
    // exit if there is no project associated to the file
    if (projectPath === null) {
      return;
    }
    ...
    if (properties.indent_style === 'space' && properties.indent_size !== atom.config.get('editor.tabLength')) {
      ...
      const notification = `Tab length is now **${this.length} virtual spaces**, saved as **${properties.indent_size} spaces**.\n\n*The editor tab length differs from the project's \`.editorconfig\` indent size, meaning tab display now fit your editor tab length preference instead of the project tab length definition. In source code, tabs are still saved as defined by the project configuration.*`;
  ...
  setIndentGuide(value) {
    this.body.classList.toggle('no-guide', value);
  },
