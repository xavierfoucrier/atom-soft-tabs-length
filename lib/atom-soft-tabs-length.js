...
export default {
  ...
  observe() {
    ...
    this.observer.add(atom.workspace.observeTextEditors((editor) => {
      ...
      // gets the current directory for this project path
      ...
      if (properties.indent_style === 'space' && properties.indent_size !== atom.config.get('editor.tabLength')) {
        ...
        // displays a notification for each open file in the editor
        if (atom.config.get('atom-soft-tabs-length.notification')) {
          atom.notifications.addInfo(`${editor.buffer.file.getBaseName()}`, {
            description: `Your editor tab length differs from the project's \`.editorconfig\` indent size.\nThis file is now displayed with tab length of **${this.length} virtual spaces** but indentation is still of **${properties.indent_size} spaces** in the source code.`,
            dismissable: true
          });
        }
      }
  ...
  setSoftTabsLength(value) {
    ...
    Array.from(this.body.querySelectorAll('atom-text-editor')).forEach((editor) => {
      editor.setAttribute('data-soft-tabs-length', this.length);
    });
  }
};
