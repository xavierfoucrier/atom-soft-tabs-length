...
export default {
  ...
  activate() {
    ...
    PackageDependencies.install('atom-soft-tabs-length', false).then(() => {
      ...
      this.body.setAttribute('style', `--soft-tabs-invisible-space:'${atom.config.get('editor.invisibles.space')}';${this.body.getAttribute('style') || ''}`);
    });
  },
  ...
  observe() {
    ...
    // observes invisibles space property
  ...
  // sets the invisibles space character
  setInvisiblesSpace(value) {
    // parse body styles and set the --soft-tabs-invisible-space property
  }
};
