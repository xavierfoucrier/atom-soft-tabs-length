...
export default {
  ...
  observe() {
    ...
    this.observer.add(atom.workspace.observeTextEditors((editor) => {
      ...
      // excludes some path depending on the user ignore pattern setting
      console.warn(file.getPath() + ' -> ' + atom.config.get('atom-soft-tabs-length.ignorePattern'));
      console.warn(new RegExp(atom.config.get('atom-soft-tabs-length.ignorePattern')).test(file.getPath()));
      // console.dir(new RegExp('js$').test('C:\Users\Copyrightx\Documents\GitHub\project-4\index.js'));
