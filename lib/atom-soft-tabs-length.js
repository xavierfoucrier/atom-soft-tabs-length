...
export default {
  ...
  observe() {
    ...
    this.observer.add(atom.config.observe('atom-soft-tabs-length.notificationScope', {}, (value) => {
      this.setNotificationScope(value);
    }));
    ...
    this.observer.add(atom.workspace.observeTextEditors((editor) => {
      ...
      if (properties.indent_style === 'space' && properties.indent_size !== atom.config.get('editor.tabLength')) {
        ...
        switch (atom.config.get('atom-soft-tabs-length.notificationScope')) {
          case 'project':
            if (typeof this.scopes[projectPath] === 'undefined') {
              this.scopes[projectPath] = true;
