...
export default {
  ...
  run(editor) {
    ...
    // get the project editor config file
    let editorConfigFile = directory.getFile('.editorconfig');

    // try to read a global editor config file, or exit if any editor config file is found
    if (!editorConfigFile.existsSync()) {
      if (atom.config.get('atom-soft-tabs-length.globalEditorConfigFile') === '') {
        return;
      }

      editorConfigFile = new File(atom.config.get('atom-soft-tabs-length.globalEditorConfigFile'));

      if (editorConfigFile.getBaseName() !== '.editorconfig') {
        atom.notifications.addError('atom-soft-tabs-length', {
          description: `You need to **provide a valid .editorconfig file**:\n*${editorConfigFile.getPath()}*`,
          dismissable: true
        });
      ...
      if (!editorConfigFile.existsSync()) {
        atom.notifications.addError('atom-soft-tabs-length', {
          description: `The .editorconfig file that you provide **does not exist**:\n*${editorConfigFile.getPath()}*`,
          dismissable: true
        });
