...
export default {
  ...
  observe() {
    ...
    // observes useEditorConfigFile property
    this.observer.add(atom.config.observe('atom-soft-tabs-length.useEditorConfigFile', {}, (value) => {
      this.setUseEditorConfigFile(value);
    }));
  },
  ...
  setSoftTabsLength(value) {
    ...
    const length = this.getSoftTabsLength();

    // exits if the soft tabs length is the same as the editor configuration
    if (length === atom.config.get('editor.tabLength')) {
      this.body.removeAttribute('data-soft-tabs-length');
      return;
    }
  ...
  // sets the use of the editor config
  setUseEditorConfigFile(value) {
    if (!value) {
      return;
    }
    ...
    atom.workspace.observeTextEditors((editor) => {
      ...
      EditorConfig.parse(editorConfigFile.getPath()).then((properties) => {
        ...
        if (properties.indent_style === 'space' && properties.indent_size !== atom.config.get('editor.tabLength')) {
          ...
          // appends the soft tabs length setting
          this.body.setAttribute('data-soft-tabs-length', this.getSoftTabsLength());
        }
      });
    });
  }
